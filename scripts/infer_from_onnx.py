...
from packnet_sfm.utils.image import load_image, flip_lr
from packnet_sfm.utils.config import parse_test_file
from packnet_sfm.utils.load import set_debug
from packnet_sfm.utils.depth import write_depth, inv2depth, viz_inv_depth, depth2inv, post_process_inv_depth
...
def infer_and_save_depth(input_file, output_file, model_wrapper, image_shape, half, save):
    ...
    if torch.cuda.is_available():
        ...
        image_lr = flip_lr(image)
        image = post_process_inv_depth(image, image_lr)
    # Depth inference (returns predicted inverse depth)
    pred_inv_depth = model_wrapper.depth(image)[0]

    # Read binary file
    # with open('C:/Users/seok436/PycharmProjects/NNEF-Tools/'
    #       'output_0.bin') as f:
    #     rectype = np.dtype(np.float32)
    #     bdata = np.fromfile(f, dtype=rectype)
    #     # bdata = bdata[32:]
    #     b_res_data = bdata.reshape((192, 640))
    #     b_res_data = to_tensor(b_res_data)
    #     pred_inv_depth = b_res_data
    ...
    if save == 'npz' or save == 'png':
        ...
        #write_depth(filename, depth=depth2inv(pred_inv_depth))
    else:
        # Prepare RGB image
        rgb = image[0].permute(1, 2, 0).detach().cpu().numpy() * 255
        # Prepare inverse depth
        viz_pred_inv_depth = viz_inv_depth(pred_inv_depth[0]) * 255
        # viz_pred_inv_depth = pred_inv_depth[0] * 255
